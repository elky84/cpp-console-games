...
void MoveCar(bool bDraw)
{
	...
	for(i=0; i<MAX; ++i) {
		if(C[i].bExist && --C[i].nStay <= 0) {
			...
			if(C[i].x == 61) { C[i].bExist = false; }
			if(C[i].x == -9) { C[i].bExist = false; }
...
void DrawBkg()
{
	...
	for(i=0 ;i<5; ++i) {
		if(bGoals[i]) {
			gotoxy(10+i*10, 3); putch('F');
		}
	}

	for(i=0; i<nLife-1; ++i) {
		gotoxy(1+i, 18); printf("%c ", 'F');
	}
	if(i==0) { gotoxy(1, 18); putch(' '); }
}
...
void MoveFrog(int key)
{
	...
	if(key == 72 && fy > 3) { --fy; }
	else if(key == 75 && fx > 1) { --fx; }
	else if(key == 77 && fx < 60) { ++fx; }
